...
#### 7.类和对象？面向对象和面向过程？构造器？成员变量和局部变量的区别？



#### 8.String、StringBuilder、StringBuffer 的异同
...
#### 9.数据库性能最佳实践？JDBC、PreparedStatement、批量、事务、锁

# MySQL

#### 1.索引？优势和劣势？索引类型？聚集索引和非聚集索引？覆盖索引？



#### 2.Hash 索引和 B+ 树索引？优缺点？MyIsam 和 InnoDB 的 B+ 树实现？
...
#### 7.锁机制和 InnoDB 算法？表级锁和行级锁？InnoDB 支持的行级锁算法？共享锁和排它锁？如何避免死锁？InnoDB 行锁实现方式？



#### 8.SQL 语句执行过程？查询？更新？为什么 redo log 使用两阶段提交？



#### 9.MVCC：多版本并发控制？作用？如何实现？



#### 10.redo log？binlog？undo log？



#### 11.MySQL 调优？Mysql 性能分析？查询优化？索引优化？锁优化？
...
#### 3.Redis 的数据结构和应用场景？String、list、hash、set、sorted set？
...
#### 5.过期时间的用处？如何判断过期？删除策略？Redis 内存淘汰机制？
...
#### 10.如何保证缓存和数据库数据的一致性？（三种读写策略）



#### 11.分布式锁的实现？SetNX + expire + Del



#### 12.Redis  事务？为什么不支持回滚？
...
#### 1.Zookeeper 是什么？有什么优势？有什么特点？应用场景？



#### 2.数据模型？数据节点？Znode 的 4 种类型？Znode 的数据结构？



#### 3.权限控制 ACL？基于 Znode？5 种操作权限？4 种身份认证方式？
...
#### 5.会话（Session）？作用？为什么选择 TCP 长连接？



#### 6.Zookeeper 集群？三种角色？Leader 服务器选举过程？为什么集群最好是单数台？



#### 7.使用场景和实现？选主？分布式锁？命名服务？集群管理？服务注册中心？



#### 8.一致性算法实现（ZAB）？写操作？Leader 选举方式？崩溃恢复？
...
#### 3.3Paxos 算法？角色？实现阶段？死循环问题？



#### 4.Raft 算法？概念？Leader 选举？日志复制？选举超时、心跳超时？发生网络分区？
